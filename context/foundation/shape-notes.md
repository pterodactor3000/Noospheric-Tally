...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: Primary pain
      decision: Not knowing current stock of cooking staples and pet food at the moment of ordering or shopping.
    - topic: Primary persona
      decision: Household grocery buyer and animal feeder, one primary user, spouse as likely second user later.
    - topic: Access model
      decision: Account-based access owning one household inventory, spouse invitable later.
    - topic: Roles
      decision: All household members equal, no owner-only permissions in the first version.
    - topic: Restock signal
      decision: User sets a minimum quantity per item; item is flagged when count reaches or falls below it.
    - topic: Smallest proof flow
      decision: Sign in, scan and identify item, add quantity, scan and subtract on use, view count and restock flag. Approved by user.
    - topic: Time budget
      decision: One month to a usable version.
    - topic: Scan direction
      decision: Persistent stocking or using mode, always visible while scanning, reset to using when the app reopens.
    - topic: Items without barcodes
      decision: Items may exist with no barcode and be adjusted by hand after a name search.
    - topic: Double scan protection
      decision: Scanning session shows a running list of recorded changes with per-line undo.
    - topic: Duplicate item prevention
      decision: Before creating an item for an unknown barcode, offer existing items so the barcode attaches to one.
    - topic: Nice-to-have scope
      decision: FR-010 below-minimum list kept in the first version; FR-011 invite deferred.
    - topic: Domain rule
      decision: Items at or below their minimum need restocking, ranked by shortfall size.
    - topic: Urgency source
      decision: Computed from shortfall only, no manual priority or category stored.
    - topic: Offline support
      decision: Dropped from the first version, recorded as an explicit later goal.
    - topic: Scan speed commitment
      decision: A few seconds per item, one confirmation per scan acceptable.
    - topic: Product form
      decision: Mobile-first web app in the phone browser, camera scanning through the browser.
    - topic: Deadline mode
      decision: Soft one-month target, after-hours work only.
    - topic: Non-goals
      decision: Expiry tracking, shop ordering, price tracking, recipe planning, storage locations, separate households, offline use, native store distribution.
    - topic: Secondary success criterion
      decision: Replaced with in-store use of the below-minimum list, because the original depended on deferred FR-011.
    - topic: Restock ranking
      decision: Shortfall-only ranking accepted for the first version, revisit after real use.
    - topic: Consumption history
      decision: Included as nice-to-have FR-015.
    - topic: Unknown barcode naming
      decision: External product catalog prefills the name, editable, with manual naming as fallback (FR-016).
    - topic: Target scale
      decision: Medium, many households eventually, while separate households stay out of the first version.
  frs_drafted: 16
...
### Current workaround and its failure

The user and his wife exchange chat messages about what to buy, and have tried note apps and other apps. Every workaround requires the user to remember to check stock at home, and the check is unavailable or unreliable at the moment of purchase.

### Distinguishing insight

Updating stock at the physical moment of change, by scanning the item's barcode, is fast enough to keep counts trustworthy. Manual lists fail because they depend on memory at a moment separated from the physical item.
...
Resolved during cross-check, kept for traceability:
...
## Forward: technical considerations

- The user asked for a web app or mobile app. Confirmed as a mobile-first web app opened in the phone browser, with camera scanning through the browser. Native store distribution is a non-goal.
- The repository currently holds an unmodified React, TypeScript, and Vite template. No stack decision is part of this discovery record.
